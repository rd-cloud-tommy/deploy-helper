...
var blockTraffic = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("Start deregister instance...")
		...
		if size > 0 {
			...
			for _, t := range targetGroups {
				go func(targetGroupArn *string) {
					defer wg.Done()
					fmt.Printf("start deregister instanceID[%s], arn[%s]\n", instanceID, *targetGroupArn)
					err := albSvc.Deregister(&instanceID, targetGroupArn)
					if err != nil {
						panic(err)
					}
					fmt.Printf("end deregister instanceID[%s], arn[%s]\n", instanceID, *targetGroupArn)
				}(t.Arn)
			}
			wg.Wait()
		}
